-- Add daily_streak column to profiles table for tracking consecutive daily reward claims
-- This column is referenced by check_achievements function for the "Daily Player" achievement

DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'profiles' AND column_name = 'daily_streak'
  ) THEN
    ALTER TABLE public.profiles ADD COLUMN daily_streak integer DEFAULT 0;
  END IF;
END $$;

COMMENT ON COLUMN public.profiles.daily_streak IS 'Number of consecutive days the player has claimed their daily reward';

-- Update claim_daily_reward function to track streak
...
COMMENT ON FUNCTION public.claim_daily_reward() IS 'Claim daily reward of 500 crystals for authenticated user. Can only be claimed once per day. Tracks consecutive daily streak.';

-- Fix check_achievements function to handle daily_streak column safely
...
BEGIN
  ...
  -- Use COALESCE to handle case where daily_streak column might not exist yet
  SELECT level, crystals, COALESCE(daily_streak, 0) INTO v_player_level, v_player_crystals, v_daily_streak
  FROM public.profiles
